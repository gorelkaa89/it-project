...
export default function SupportDashboardPage({ user, onLogout, onOpenTicket }) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5 px-4 py-8 text-slate-900 dark:text-slate-100">
      <header className="rounded-xl bg-white p-6 shadow dark:bg-slate-900 flex justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => load()} className="rounded border px-3 py-2 text-sm dark:border-slate-700">Обновить</button>
          <button onClick={onLogout} className="rounded border px-3 py-2 text-sm dark:border-slate-700">Выйти</button>
        </div>
      </header>

      <section className="rounded-xl bg-white p-6 shadow dark:bg-slate-900">
        <div className="flex gap-2">
          <button onClick={() => switchMode('all')} className={`rounded px-3 py-2 text-sm ${mode === 'all' ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-slate-700'}`}>Общие заявки</button>
          <button onClick={() => switchMode('my')} className={`rounded px-3 py-2 text-sm ${mode === 'my' ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-slate-700'}`}>Мои заявки</button>
        </div>
